# Disaster Recovery Environment - Infrastructure Deployment
...
locals {
  ...
  name_prefix = "${var.project_name}-disaster-recovery"
  ...
  common_tags = merge(
    ...
    var.additional_tags,
    {
      ...
      RecoveryTimeObjective = var.recovery_time_objective
      RecoveryPointObjective = var.recovery_point_objective
      ReplicationPolicy     = "Continuous"
    }
  )
}
