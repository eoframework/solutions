...
#------------------------------------------------------------------------------
# AWS Provider - DR Region (Primary for this environment)
#------------------------------------------------------------------------------
provider "aws" {
  region = var.aws.region  # DR region (us-west-2)
...
#------------------------------------------------------------------------------
# AWS Provider - Primary Region (for cross-region resources)
#------------------------------------------------------------------------------
provider "aws" {
  alias  = "primary"
  region = var.aws.dr_region  # Primary region (us-east-1)

  default_tags {
    tags = {
      Solution    = var.solution.name
      Environment = "${basename(path.module)}-primary"
      ManagedBy   = "terraform"
    }
  }
}
